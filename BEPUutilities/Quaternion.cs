...
namespace BEPUutilities
{
    ...
    public struct Quaternion : IEquatable<Quaternion>
    {
        ...
        public static void Multiply(ref Quaternion q, double scale, out Quaternion result)
        {
            result.X = q.X * scale;
            result.Y = q.Y * scale;
            result.Z = q.Z * scale;
        ...
        public static void CreateFromRotationMatrix(ref Matrix3x3 r, out Quaternion q)
        {
            ...
            if (trace >= 0)
            {
                var S = (double)Math.Sqrt(trace + 1.0) * 2; // S=4*qw 
                var inverseS = 1 / S;
                q.W = 0.25f * S;
            ...
            {
                var S = (double)Math.Sqrt(1.0 + r.M11 - r.M22 - r.M33) * 2; // S=4*qx 
                var inverseS = 1 / S;
            ...
            {
                var S = (double)Math.Sqrt(1.0 + r.M22 - r.M11 - r.M33) * 2; // S=4*qy
                var inverseS = 1 / S;
            ...
            {
                var S = (double)Math.Sqrt(1.0 + r.M33 - r.M11 - r.M22) * 2; // S=4*qz
                var inverseS = 1 / S;
        ...
        {
            double inverse = (double)(1 / Math.Sqrt(quaternion.X * quaternion.X + quaternion.Y * quaternion.Y + quaternion.Z * quaternion.Z + quaternion.W * quaternion.W));
        ...
        {
            double inverse = (double)(1 / Math.Sqrt(X * X + Y * Y + Z * Z + W * W));
            X *= inverse;
            Y *= inverse;
            Z *= inverse;
            W *= inverse;
        ...
        public static void Slerp(ref Quaternion start, ref Quaternion end, double interpolationAmount, out Quaternion result)
        {
            ...
            result.X = (double)(start.X * aFraction + end.X * bFraction);
            result.Y = (double)(start.Y * aFraction + end.Y * bFraction);
            result.Z = (double)(start.Z * aFraction + end.Z * bFraction);
            result.W = (double)(start.W * aFraction + end.W * bFraction);
        ...
        public static Quaternion Slerp(Quaternion start, Quaternion end, double interpolationAmount)
        {
            Quaternion toReturn;
        ...
        {
            double inverseSquaredNorm = quaternion.X * quaternion.X + quaternion.Y * quaternion.Y + quaternion.Z * quaternion.Z + quaternion.W * quaternion.W;
        ...
        public static void Transform(ref Vector3 v, ref Quaternion rotation, out Vector3 result)
        {
            ...
            double transformedX = v.X * (1f - yy2 - zz2) + v.Y * (xy2 - wz2) + v.Z * (xz2 + wy2);
            double transformedY = v.X * (xy2 + wz2) + v.Y * (1f - xx2 - zz2) + v.Z * (yz2 - wx2);
            double transformedZ = v.X * (xz2 - wy2) + v.Y * (yz2 + wx2) + v.Z * (1f - xx2 - yy2);
        ...
        /// <param name="rotation">Rotation to apply to the vector.</param>
        /// <param name="result">Transformed vector.</param>
        public static void TransformX(double x, ref Quaternion rotation, out Vector3 result)
        {
            //This operation is an optimized-down version of v' = q * v * q^-1.
            //The expanded form would be to treat v as an 'axis only' quaternion
            //and perform standard quaternion multiplication.  Assuming q is normalized,
        ...
        /// <param name="rotation">Rotation to apply to the vector.</param>
        /// <param name="result">Transformed vector.</param>
        public static void TransformY(double y, ref Quaternion rotation, out Vector3 result)
        {
            //This operation is an optimized-down version of v' = q * v * q^-1.
            //The expanded form would be to treat v as an 'axis only' quaternion
            //and perform standard quaternion multiplication.  Assuming q is normalized,
        ...
        /// <param name="rotation">Rotation to apply to the vector.</param>
        /// <param name="result">Transformed vector.</param>
        public static void TransformZ(double z, ref Quaternion rotation, out Vector3 result)
        {
            //This operation is an optimized-down version of v' = q * v * q^-1.
            //The expanded form would be to treat v as an 'axis only' quaternion
            //and perform standard quaternion multiplication.  Assuming q is normalized,
        ...
        public static void CreateFromAxisAngle(ref Vector3 axis, double angle, out Quaternion q)
        {
            double halfAngle = angle * 0.5;
            double s = Math.Sin(halfAngle);
            q.X = (double)(axis.X * s);
        ...
        public static void CreateFromYawPitchRoll(double yaw, double pitch, double roll, out Quaternion q)
        {
            ...
            q.X = (double)(cosYawSinPitch * cosRoll + sinYawCosPitch * sinRoll);
            q.Y = (double)(sinYawCosPitch * cosRoll - cosYawSinPitch * sinRoll);
            q.Z = (double)(cosYawCosPitch * sinRoll - sinYawSinPitch * cosRoll);
            q.W = (double)(cosYawCosPitch * cosRoll + sinYawSinPitch * sinRoll);
        ...
        public static double GetAngleFromQuaternion(ref Quaternion q)
        {
        ...
        public static void GetAxisAngleFromQuaternion(ref Quaternion q, out Vector3 axis, out double angle)
        {
#if !WINDOWS
            axis = new Vector3();
#endif
            ...
            {
                Vector3.Divide(ref axis, (double)Math.Sqrt(lengthSquared), out axis);
                angle = 2 * (double)Math.Acos(MathHelper.Clamp(qw, -1, 1));
            }
            else
            {
