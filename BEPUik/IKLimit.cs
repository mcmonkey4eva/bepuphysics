...
namespace BEPUik
{
    ...
    public abstract class IKLimit : IKJoint
    {
        ...
        protected internal override void SolveVelocityIteration()
        {
            ...
            double impulseSquared = accumulatedImpulse.LengthSquared();
            if (impulseSquared > maximumImpulseSquared)
            {
                //Oops! Clamp that down.
                Vector3.Multiply(ref accumulatedImpulse, maximumImpulse / (double)Math.Sqrt(impulseSquared), out accumulatedImpulse);
            }
