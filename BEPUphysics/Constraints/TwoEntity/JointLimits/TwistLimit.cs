...
namespace BEPUphysics.Constraints.TwoEntity.JointLimits
{
    ...
    public class TwistLimit : JointLimit, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public TwistLimit(Entity connectionA, Entity connectionB, Vector3 axisA, Vector3 axisB, double minimumAngle, double maximumAngle)
        {
        ...
        public override double SolveIteration()
        {
            ...
            double lambda = -(velocityA + velocityB) + biasVelocity - softness * accumulatedImpulse;
        ...
        public override void Update(double dt)
        {
            ...
            double distanceFromCurrent, distanceFromMaximum;
        ...
        private static double ComputeAngleError(double distanceFromCurrent, double distanceFromMaximum)
        {
            double errorFromMin = MathHelper.TwoPi - distanceFromCurrent;
            double errorFromMax = distanceFromCurrent - distanceFromMaximum;
            return errorFromMax > errorFromMin ? errorFromMin : -errorFromMax;
        }

        private double GetDistanceFromMinimum(double angle)
        {
            if (minimumAngle > 0)
            {
        ...
        private bool IsAngleValid(double currentAngle, out double distanceFromCurrent, out double distanceFromMaximum)
        {
