...
namespace BEPUphysics.Character
{
    ...
    public class SupportFinder
    {
        ...
        double maximumAssistedDownStepHeight = 1;
        /// <summary>
        ...
        /// </summary>
        public double MaximumAssistedDownStepHeight
        {
            get
            {
        ...
        private void UpdateSupportData(ref Vector3 down)
        {
            ...
            if (contacts.Count > 1)
            {
                ...
                double length = supportData.Normal.LengthSquared();
                ...
                {
                    Vector3.Multiply(ref supportData.Normal, 1 / (double)Math.Sqrt(length), out supportData.Normal);
                }
            }
        ...
        private void UpdateVerticalSupportData(ref Vector3 down, ref Vector3 movementDirection)
        {
            if (HasTraction)
            {
                if (tractionContacts.Count > 0)
                {
                    ...
                    int greatestIndex = -1;
                    double greatestDot = -double.MaxValue;
        ...
        public void UpdateSupports(ref Vector3 movementDirection)
        {
            ...
            double rayCastInnerRadius = Math.Max((extremePoint.X + convexShape.collisionMargin) * 0.8f, extremePoint.X);
            ...
            {
                double supportRayLength = maximumAssistedDownStepHeight + BottomDistance;
                SupportRayData = null;
        ...
        bool TryDownCast(ref Ray ray, double length, out bool hasTraction, out SupportRayData supportRayData)
        {
            RayHit earliestHit;
            ...
            {
                double lengthSquared = earliestHit.Normal.LengthSquared();
                if (lengthSquared < Toolbox.Epsilon)
                {
                    //Don't try to continue if the support ray is stuck in something.
                    return false;
                }
                Vector3.Divide(ref earliestHit.Normal, (double)Math.Sqrt(lengthSquared), out earliestHit.Normal);
